...
def get_creased(input_file,output_directory,ifWrinkles=False,ifCreases=False,crease_angle=0,num_creases_vertically=3,num_creases_horizontally=2,bbox=False):
    ...
    # read wrinkle image as grayscale and convert to float in range 0 to 1
        wrinkle_dir = os.path.join('CreasesWrinkles', 'wrinkles-dataset')
        wrinkle_file_name = os.path.join(
            wrinkle_dir,
            random.choice([f for f in os.listdir(wrinkle_dir) 
                           if os.path.isfile(os.path.join(wrinkle_dir, f)) and f.lower().endswith(('.png', '.jpg', '.jpeg'))])
        )
